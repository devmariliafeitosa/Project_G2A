...
// Cell as its own component so hover state is isolated
const GridCell = ({
  allocation, subject, teacher, day, slot, onDelete, onAdd
}: {
  allocation?: ScheduleEntry;
  subject?: Subject;
  teacher?: User;
  day: string;
  slot: typeof ACTIVE_SLOTS[0];
  onDelete: (id: string) => void;
  onAdd: (day: string, slotId: string) => void;
}) => {
  const [hovered, setHovered] = useState(false);

  if (allocation && subject && teacher) {
    return (
      <div
        onMouseEnter={() => setHovered(true)}
        onMouseLeave={() => setHovered(false)}
        className={`relative p-4 rounded-2xl border transition-all shadow-sm ${subject.color || PASTEL_COLORS[0]} ${hovered ? 'scale-[1.02] shadow-md' : ''}`}
      >
        <button
          onClick={(e) => {
            e.stopPropagation();
            if (window.confirm(`Deseja remover a alocação de ${subject.name}?`)) {
              onDelete(allocation.id);
            }
          }}
          className={`absolute top-2 right-2 w-6 h-6 rounded-full bg-white/80 hover:bg-rose-500 hover:text-white flex items-center justify-center text-zinc-600 transition-all shadow-sm border border-black/10 ${hovered ? 'opacity-100' : 'opacity-0'}`}
  ...
  return (
    <button
      ...
      className="w-full h-[72px] border-2 border-dashed border-zinc-100 hover:border-[#32a041]/30 rounded-2xl flex items-center justify-center text-zinc-300 hover:text-[#32a041] hover:bg-[#32a041]/5 transition-all"
...
const WeeklyGrid = ({ allocations, subjects, teachers, onDeleteAllocation, onOpenAddModal }: WeeklyGridProps) => (
        ...
        {ACTIVE_SLOTS.map(slot => (
          <tr key={slot.id} className="border-b border-zinc-50 hover:bg-zinc-50/20 transition-all">
            ...
            {DAYS.map(day => {
              ...
              const subject = allocation ? subjects.find(s => s.id === allocation.subjectId) : undefined;
              const teacher = allocation ? teachers.find(t => t.id === allocation.teacherId) : undefined;
              return (
