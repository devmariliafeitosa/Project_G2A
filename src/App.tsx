...
type AdminViewName =
  | 'dashboard' | 'courses' | 'teachers' | 'settings' | 'reports'
  | 'notifications' | 'occurrences' | 'professor_schedule' | 'professor_preferences';
...
<<<<<<< HEAD
  if (isCoordinator(session.user)) {
    ...
    const renderCoordView = () => {
      switch (coordView) {
        ...
        case 'allocations':  return <AllocationsView user={session.user} teachers={teachers} subjects={subjects} courses={courses} semesters={semesters} allocations={allocations} onAddAllocation={e => setAllocations(p => [...p, e])} onDeleteAllocation={id => setAllocations(p => p.filter(a => a.id !== id))} />;
        ...
        case 'my-subjects':  return <EmptyState title="Minhas Disciplinas" />;
  ...
  const navigate = (view: AdminViewName) => { setAdminView(view); setSelectedTeacherId(null); };

  const renderAdminView = () => {
    if (selectedTeacherId) return <TeacherProfileView teacher={teachers.find(t => t.id === selectedTeacherId)!} onBack={() => setSelectedTeacherId(null)} onUpdate={updateTeacher} currentUserRole={session.user?.role} currentUserId={session.user?.id} onLogout={handleLogout} />;
    switch (adminView) {
      case 'dashboard':               return <DashboardView user={session.user!} stats={{ teachers: teachers.length, courses: courses.length, subjects: subjects.length }} courses={courses} subjects={subjects} teachers={teachers} schedules={schedules} onAddSchedule={addSchedule} />;
      case 'courses':                 return <CoursesView courses={courses} teachers={teachers} subjects={subjects} onAddCourse={addCourse} onAddSubject={addSubject} onUpdateCourse={updateCourse} onDeleteSubject={deleteSubject} onUpdateSubject={updateSubject} />;
      case 'teachers':
        return (
          <TeachersView
            user={session.user}
      ...
      case 'reports':                 return <ReportsView courses={courses} teachers={teachers} subjects={subjects} schedules={schedules} reportHistory={reportHistory} onAddHistory={addReportToHistory} />;
      case 'occurrences':             return <TeacherOccurrencesView occurrences={occurrences} teachers={teachers} subjects={subjects} onCreateOccurrence={createOccurrence} onUpdateOccurrence={updateOccurrence} />;
      case 'professor_schedule':      return <ProfessorScheduleView user={session.user!} courses={courses} subjects={subjects} teachers={teachers} schedules={schedules} />;
      case 'professor_preferences':
          return (
            ...
            />
              ...
              case 'notifications':           return <NotificationsView notifications={notifications} onUpdateNotification={updateNotification} onDeleteNotification={deleteNotification} onNavigate={path => setAdminView(path as AdminViewName)} />;
              default:                        return null;
            }
          };
  ...
  return (
    <MainLayout
      ...
      sidebar={<Sidebar user={session.user!} currentView={adminView} selectedTeacherId={selectedTeacherId} notifications={notifications} onNavigate={navigate} onSelectProfile={() => setSelectedTeacherId(session.user?.id || null)} onLogout={handleLogout} />}
    >
      {renderAdminView()}
    </MainLayout>
=======
