    ...
    <div className="space-y-10 animate-in fade-in duration-500 pb-20">
      ...
      {coordinatedCourse && (
        <div className="bg-white rounded-[2rem] border border-zinc-100 p-6 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-6 text-zinc-400 text-xs font-semibold sm:border-l sm:border-zinc-100 sm:pl-6">
            ...
            <div>
              <span className="block text-[9px] font-black uppercase text-zinc-400 tracking-widest leading-none mb-1">Créditos de Aula</span>
              <span className="text-zinc-700 font-extrabold">
                {courseSubjects.reduce((sum, curr) => sum + curr.workload, 0)}h
              </span>
            </div>
          </div>
        </div>
      ...
      {courseSubjects.length === 0 ? (
        ...
        <div className="bg-white rounded-3xl border border-zinc-100 shadow-xs overflow-hidden">
          ...
          <div className="p-8 border-b border-zinc-100 bg-zinc-50/20 flex flex-col md:flex-row justify-between items-stretch md:items-center gap-6">
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-black uppercase text-zinc-400 tracking-wider">Filtrar Semestre:</span>
              </div>
              <select
                value={selectedPeriod}
                onChange={(e) => setSelectedPeriod(e.target.value)}
                className="h-12 px-5 bg-white border border-zinc-200 rounded-2xl text-xs font-black uppercase tracking-wider outline-none focus:border-[#32a041]/30 text-zinc-700 shadow-sm"
              >
          ...
          <div className="px-8 py-4 border-b border-zinc-50 bg-[#32a041]/5 flex flex-wrap gap-6 items-center text-[10px] font-black uppercase tracking-wider text-emerald-800">
            <span className="flex items-center gap-1.5">
              <span className="w-1.5 h-1.5 rounded-full bg-emerald-500"></span>
              {stats.total} Disciplinas Registradas
            </span>
            <span className="flex items-center gap-1.5">
            ...
            {selectedPeriod !== 'all' && (
              <span className="ml-auto bg-emerald-100 hover:bg-emerald-200/60 px-2 py-0.5 rounded text-[8px]">
                Filtrado pelo {selectedPeriod}º Semestre
              </span>
            )}
          </div>
