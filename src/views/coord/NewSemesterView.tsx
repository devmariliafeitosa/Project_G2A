      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-7 bg-white rounded-[2.5rem] border border-zinc-100 shadow-xl shadow-zinc-150/5 p-8 md:p-10 space-y-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {/* 5. VÍNCULO DE SEMESTRES ATIVOS NO PERÍODO LETIVO */}
            <div className="space-y-3 pt-3 border-t border-zinc-100">
              <div>
                <label className="text-[10px] font-black uppercase tracking-widest text-[#32a041] block">
                  Vínculo de Semestres Ativos (Ativação Concorrente) *
                </label>
                <p className="text-[10px] text-zinc-400 font-semibold italic mt-0.5">
                  Selecione quais semestres acadêmicos rodarão em paralelo neste período civil ({civilPeriod}).
                </p>
              </div>
                  ...
                  return (
                    <label 
                      ...
                      className={`flex items-center justify-between p-2.5 rounded-xl border transition-all text-left ${
                        !hasOffer
                          ? 'bg-zinc-100/50 border-zinc-200 opacity-60 cursor-not-allowed select-none'
                          : isChecked
                            ? 'bg-white border-emerald-350 hover:bg-emerald-50/10 cursor-pointer shadow-xs'
                            : 'bg-white border-zinc-200 hover:bg-zinc-50/50 cursor-pointer'
                      }`}
                    >
                      <div className="flex items-center gap-3">
                        <input
                          ...
                          onChange={(e) => {
                            if (!hasOffer) return;
                            if (e.target.checked) {
                              setActiveSemesters(prev => [...prev, num]);
                              triggerToast(`${num}º Semestre ativado no período civil ${civilPeriod}.`, 'success');
                            } else {
                              setActiveSemesters(prev => prev.filter(n => n !== num));
                              triggerToast(`${num}º Semestre inativado.`, 'info');
                            }
                          }}
                          className="w-4 h-4 rounded text-[#32a041] focus:ring-[#32a041]/30 border-zinc-300 pointer-events-auto"
                        />
                        <div>
                          <span className={`text-[11px] font-black block leading-none ${isChecked && hasOffer ? 'text-zinc-800' : 'text-zinc-505'}`}>
                            {num}º Semestre Letivo
                          </span>
                          <span className="text-[9px] text-zinc-400 font-semibold block mt-0.5 italic">
                            {num === matrixSemester 
                              ? `Editando configurações atuais de disciplinas` 
                              : `Possui ofertas estruturadas prontas para simulação e alocação`}
                          </span>
                        </div>
                      </div>

                      {!hasOffer && (
                        <span className="text-[8px] font-extrabold uppercase text-zinc-400 bg-zinc-100 border border-zinc-200 px-1.5 py-0.5 rounded leading-none">
                          Vazio / Sem Ofertas
                        </span>
                      )}
                    </label>
              ...
              {/* Informational Warning constraints */}
              <div className="flex items-center gap-2 pl-1">
                ...
                <p className="text-[9px] text-zinc-400 font-bold uppercase tracking-wider">
                  Configuração protegida: apenas semestres com disciplinas associadas podem ser ativados.
                </p>
              </div>
            </div>
        ...
        <div className="lg:col-span-5 space-y-6">
          <div className="bg-white rounded-[2.5rem] border border-zinc-100 shadow-xl shadow-zinc-150/5 p-8 flex flex-col h-full min-h-[480px]">
        ...
        {periodsGrouped.map((period) => {
          ...
          const totalSubjectsNum = period.semestersList.reduce(
            (sum, s) => sum + (s.offeredSubjectIds?.length || 0),
            0
          );

          const semestersCreatedNumbers = period.semestersList
            .map(s => s.matrixSemester || 1)
            .sort((a, b) => a - b);

          const activeMatrixOnes = period.semestersList
          ...
          return (
            <SemesterCard
              ...
              onDeletePeriod={
                onDeleteSemester
                  ? (ident) => setDeletingPeriodIdent(ident)
                  : undefined
              }
            />
          );
            ...
            <div className="p-4 bg-zinc-50 rounded-2xl border border-zinc-100 mt-6 space-y-1.5">
              <span className="text-[8px] font-black uppercase tracking-widest text-[#32a041] leading-none block">DIRETRIZES DE AUDITORIA</span>
              <p className="text-[10px] font-medium leading-relaxed text-zinc-400 uppercase tracking-wider">
                O salvamento das ofertas limpa o módulo de preferências (DOC-002) e reinicia o banco de preferências zerado para o período civil correspondente. A exclusão de períodos não apaga registros de simulações históricas.
              </p>
            </div>
